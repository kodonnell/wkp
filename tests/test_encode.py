...
def test_known_encodes(input, n, precision, expected):
    assert encode.encode_floats(input, n, precision) == expected.encode("ascii")
...
def test_known_encode3():
    ...
    s = encode.encode_floats(input, 3, (6, 6, 0))
    decoded = encode.decode_floats(s, 3, (6, 6, 0))
...
def test_multi_encode_decode_noop():
    ...
    encoded = encode.encode_floats(out, 3, [2, 3, 4])
    decoded = encode.decode_floats(encoded, 3, [2, 3, 4])
    assert out == decoded
...
def test_multi_matches_polyline_package():
    ...
    encoded = encode.encode_floats(out, 2, [2, 2])
...
def test_encode_array_matches_list():
    ...
    encoded_from_list = encode.encode_floats(out, 2, [2, 2])
    encoded_from_array = encode.encode_floats_array(arr, 2, [2, 2])
...
def test_encode_array_bytes_roundtrip():
    ...
    encoded = encode.encode_floats_array(arr, 2, [2, 2])
    decoded = encode.decode_floats(encoded, 2, [2, 2])
