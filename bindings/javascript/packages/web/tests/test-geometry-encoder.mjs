...
test('workspace roundtrip for all geometry types (web wasm)', async () => {
    ...
    const geometries = [
        { type: 'Point', coordinates: [174.776, -41.289], fn: wkp.encodePoint },
        { type: 'LineString', coordinates: [[174.776, -41.289], [174.777, -41.29], [174.778, -41.291]], fn: wkp.encodeLineString },
        { type: 'Polygon', coordinates: [[[0, 0], [1, 0], [1, 1], [0, 1], [0, 0]]], fn: wkp.encodePolygon },
        { type: 'MultiPoint', coordinates: [[0, 0], [1, 1]], fn: wkp.encodeMultiPoint },
        { type: 'MultiLineString', coordinates: [[[0, 0], [1, 1]], [[2, 2], [3, 3]]], fn: wkp.encodeMultiLineString },
        {
            type: 'MultiPolygon',
            coordinates: [
                [[[0, 0], [1, 0], [1, 1], [0, 1], [0, 0]]],
...
test('decodeHeader and convenience no-workspace path (web wasm)', async () => {
    const wkp = await createWkp();
    const geometry = { type: 'LineString', coordinates: [[0.1, 0.2], [1.1, 1.2], [2.1, 2.2]] };

    const encoded = wkp.encodeLineString(geometry, 6);
...
test('known polyline vectors match expected encodings (web wasm)', async () => {
    ...
    for (const vector of vectors) {
        const encodedBytes = wkp.encodeF64(Float64Array.from(vector.values), vector.dimensions, vector.precisions);
...
test('known 3d mixed precision value decodes without overflow (web wasm)', async () => {
    ...
    const encodedBytes = wkp.encodeF64(Float64Array.from(values), 3, precisions);
    const decoded = wkp.decodeF64(encodedBytes, 3, precisions);
